...
test_random: build tests/test_random.c
	gcc -o tests/test_random tests/test_random.c -L$(LIB_DIR) -l$(LIB_NAME)

run: test_alloc
	LD_LIBRARY_PATH=$(LIB_DIR) ./tests/test_alloc

run_random: test_random
	LD_LIBRARY_PATH=$(LIB_DIR) ./tests/test_random

clean:
	cargo clean
	rm -f tests/test_alloc tests/test_alloc_dbg tests/test_random
